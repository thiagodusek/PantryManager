...
class ProductRepositoryImpl @Inject constructor(
    ...
    override suspend fun deleteProducts(ids: List<Long>, userId: String) {
        ids.forEach { dao.deleteProductById(it, userId) }
    }
    
    override suspend fun findOrCreateProductByEan(ean: String): Product? {
        // Room implementation requires userId to filter; as a fallback we pass empty userId,
        // since the DI binds Firebase repository for actual usage.
        val entity = dao.getProductByEan(ean, userId = "").firstOrNull()
