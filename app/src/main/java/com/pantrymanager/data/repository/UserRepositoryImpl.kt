...
class UserRepositoryImpl @Inject constructor(
    ...
    override suspend fun loginWithEmailAndPassword(email: String, password: String): Result<User> {
        return try {
            ...
            // Força logout completo para limpar qualquer token em cache
            firebaseAuth.signOut()
            
            // Aguarda um momento para garantir que o logout foi processado
            kotlinx.coroutines.delay(500)
            
            // Força reautenticação com as credenciais mais recentes
            val authResult = firebaseAuth.signInWithEmailAndPassword(email, password).await()
            val firebaseUser = authResult.user ?: throw Exception("Usuário não encontrado")
            
            // Força reload do usuário para garantir dados atualizados do servidor
            firebaseUser.reload().await()
            
            // Verifica se o token é válido (força verificação no servidor)
            ...
            android.util.Log.e("UserRepository", "Erro no login: ${e.message}", e)
            
            // Trata erros específicos relacionados ao reset de senha
            val errorMessage = when {
                e.message?.contains("invalid-credential", ignoreCase = true) == true -> {
                    "Credenciais inválidas. Se você redefiniu a senha recentemente, aguarde alguns minutos e tente novamente."
                }
                e.message?.contains("wrong-password", ignoreCase = true) == true -> {
                    "Senha incorreta. Se você redefiniu a senha recentemente, verifique se está usando a nova senha."
                }
                e.message?.contains("too-many-requests", ignoreCase = true) == true -> {
                    "Muitas tentativas. Aguarde alguns minutos antes de tentar novamente."
                }
                else -> e.message ?: "Erro no login"
            }
